...
public class RoomManager : MonoBehaviourPunCallbacks
{
    ...
    public TextMeshProUGUI OccupancyRateText_ForShooting;
    ...
    public void OnEnterButtonClicked_Shooting()
    {
        mapType = MultiplayerVRConstants.MAP_TYPE_VALUE_SHOOTING;
        ExitGames.Client.Photon.Hashtable expectedCustomRoomProperties = new ExitGames.Client.Photon.Hashtable() { { MultiplayerVRConstants.MAP_TYPE_KEY, mapType } };
        PhotonNetwork.JoinRandomRoom(expectedCustomRoomProperties, 0);
    }
    #endregion
    ...
    public override void OnJoinedRoom()
    {
        ...
        if (PhotonNetwork.CurrentRoom.CustomProperties.ContainsKey(MultiplayerVRConstants.MAP_TYPE_KEY))
        {
            ...
            if (PhotonNetwork.CurrentRoom.CustomProperties.TryGetValue(MultiplayerVRConstants.MAP_TYPE_KEY, out mapType))
            {
                ...
                else if ((string)mapType == MultiplayerVRConstants.MAP_TYPE_VALUE_SHOOTING)
                {
                    // Load the Shooting Scene
                    PhotonNetwork.LoadLevel("World_Shooting");
                }
            }
        }
    }
    ...
    public override void OnRoomListUpdate(List<RoomInfo> roomList)
    {
        if (roomList.Count == 0)
        {
            ...
            OccupancyRateText_ForShooting.text = 0 + " / " + 20;
        }
        ...
        foreach (RoomInfo room in roomList)
        {
            ...
            else if (room.Name.Contains(MultiplayerVRConstants.MAP_TYPE_VALUE_SHOOTING))
            {
                Debug.Log("Room is a Shooting map. Player count is: " + room.PlayerCount);
                // Update the shooting room occupancy field
                OccupancyRateText_ForShooting.text = room.PlayerCount + " / " + 20;
            }
        }
    }
